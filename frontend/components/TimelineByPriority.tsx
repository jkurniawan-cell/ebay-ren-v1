        ...
        <table className="min-w-full border-collapse bg-white shadow-sm">
          <thead className="sticky top-0 z-20">
            <tr className="border-b border-gray-300 bg-gradient-to-r from-gray-50 to-white">
              <th className="px-2 py-1 text-xs font-semibold text-white uppercase w-[150px] bg-gray-700">
                M0 Priority
              </th>
              <th className="px-2 py-1 text-xs font-semibold text-white uppercase w-[150px]" style={{ backgroundColor: '#78909c' }}>
                M1
              </th>
              {quarters.map((quarter) => (
                <th
                  key={quarter}
                  className="px-1 py-1 text-xs font-semibold text-center text-white min-w-[90px]"
          ...
          <tbody>
            ...
            {sortedMarkets.map((market) => {
              ...
              Object.entries(marketGroups[market])
                .sort(([m1A], [m1B]) => m1A.localeCompare(m1B))
                .forEach(([normalizedM1Name, groupData]) => {
              ...
              return (
                <React.Fragment key={market}>
                  {/* Market Header Row */}
                  <tr>
                    <td colSpan={2 + quarters.length} className="bg-gray-100 border-b border-gray-300 sticky z-10 py-1.5" style={{ top: '28px' }}>
                      <div className="flex items-center justify-center">
                        <div className="text-xs font-semibold text-gray-900 bg-gray-200 px-4 py-0.5 rounded flex items-center gap-2">
                      ...
                      return (
                        ...
                        >
                          ...
                          {isFirstInGroup && (
                            ...
                            >
                              <div className="text-xs font-semibold leading-tight text-white" style={{ lineHeight: '1.2' }}>
                                {displayPriority}
                              </div>
                            </td>
                          )}
